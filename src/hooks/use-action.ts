...
function useAction<TAction extends ActionFunction, TSchema extends ZodSchema>(
...
interface UseActionFormOptions<TAction extends ActionFunction, TSchema extends ZodSchema> extends UseActionOptions<TAction> {
  schema: TSchema;
  onFormSubmit?: (event: FormEvent<HTMLFormElement>, data: z.infer<TSchema>) => void;
  defaultValue: DefaultValue<TSchema>;
}

function useActionForm<TAction extends ActionFunction, TSchema extends ZodSchema>(options: UseActionFormOptions<TAction, TSchema> & {
  schema: TSchema
}) {
